...
from utils.mst_utils import increase_cost_for_outside_bone, primMST, primMST_symmetry, loadSkel_recur, inside_check, flip
...
def create_single_data(mesh_filaname):
    ...
    if not os.path.exists(mesh_filaname.replace('_remesh.obj', '_normalized.binvox')):
        if platform == "linux" or platform == "linux2":
            os.system("chmod +x ./binvox ; ./binvox -d 88 -pb " + mesh_filaname.replace("_remesh.obj", "_normalized.obj"))
        elif platform == "win32":
...
def predict_joints(input_data, vox, joint_pred_net, threshold, bandwidth=None, mesh_filename=None, symm=False):
    """
    Predict joints
...
def predict_skeleton(input_data, vox, root_pred_net, bone_pred_net, mesh_filename, symm=True):
    """
    ...
    if symm:
        ...
        parent, key, root_id = primMST(cost_matrix, root_id)
...
def tranfer_to_ori_mesh(filename_ori, filename_remesh, pred_rig):
    ...
    # print(vert_ori.shape,vert_ori)
    # vert_ori_original = trimesh.load(filename_ori).vertices
    # print(vert_ori_original.vertices.shape,vert_ori_original.vertices)
    # print('numpy array equal',np.allclose(vert_ori, vert_ori_original))
    # for i in range(vert_ori.shape[0]):
    #     if not np.allclose(vert_ori[i],vert_ori_original[i]):
    #         print('wrong')
    #         print(vert_ori[i],vert_ori_original[i])
    ...
    # print('length',tranfer_rig.joint_skin,len(tranfer_rig.joint_skin))
    return tranfer_rig


def extract_connections(pred_skel):
    # draw mesh
    root = pred_skel.root
    this_level = root.children
    joint_idx = 0
    connections = {'root': [root.pos, [node.name for node in this_level], joint_idx]}

    # print('connections for root is',connections)
    ...
    while this_level:
        ...
        for p_node in this_level:
            joint_idx += 1
            connections[p_node.name] = [p_node.pos, [node.name for node in p_node.children], joint_idx]
            # print('connections for root is',connections)
            ...
            # print(p_node.name,p_node.pos,p_node.children)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='extract skeleton')
    parser.add_argument('--output_path', default='',
                        help='path to output pickle file')
    parser.add_argument('--mesh_path', default='',
                        help='path to the mesh to be used')
    parser.add_argument('--mesh_name', default='ama_skeleton',
                        help='specify desired mesh name to be used')
    ...
    ##### Parameters to tune the initial kinematic chain #####
    # increase bandwidth for more sparse kinematic chain joints
    # generally a good value of bandwidth is smaller than 0.05
    model_id, bandwidth, threshold = args.mesh_name, 0.043, 1e-6

    # create data used for inferece
    if args.mesh_path != '':
        mesh_ori = o3d.io.read_triangle_mesh(args.mesh_path)
        mesh_ori = mesh_ori.simplify_quadric_decimation(10000)
        o3d.io.write_triangle_mesh(os.path.join(input_folder, '{:s}_remesh.obj'.format(model_id)), mesh_ori)
        print("save remeshed object for model ID {:s}".format(model_id))

    print("creating data for model ID {:s}".format(model_id))
    mesh_filename = os.path.join(input_folder, '{:s}_remesh.obj'.format(model_id))
    print('mesh used:',mesh_filename)
    ...
    data = predict_joints(data, vox, jointNet, threshold, bandwidth=bandwidth,
                          mesh_filename=mesh_filename.replace("_remesh.obj", "_normalized.obj"),symm=args.symm)
    data.to(device)
    print("predicting connectivity")
    pred_skeleton = predict_skeleton(data, vox, rootNet, boneNet,
                                     mesh_filename=mesh_filename.replace("_remesh.obj", "_normalized.obj"), symm=args.symm)
    # extract_connections(pred_skeleton)
    pred_skeleton2 = copy.deepcopy(pred_skeleton)
    pred_skeleton2.normalize(scale_normalize, -translation_normalize)
    extracted = extract_connections(pred_skeleton2)
    with open(args.output_path, 'wb') as f:
        pickle.dump(extracted, f)
    print('extracted skeleton:',extracted)
    print("Done!")
